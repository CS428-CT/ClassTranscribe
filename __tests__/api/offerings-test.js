...
import { ENDPOINTS as OFFERING_ENDPOINTS } from '../../src/api/offerings';
import { HTTP_STATUS_CODES } from '../../src/api'
import { OFFERINGS_RESPONSE_1, OFFERINGS_RESPONSE_2, SIGN_IN_RESPONSE, STARRED_OFFERINGS_RESPONSE } from './mock-response'
import { getOfferingData, getStarredOfferings, getStarredOfferingsData } from '../../src/api/offerings'
...
describe('Get offerings data', () => {
  ...
  test('when authenticated', async () => {
      ...
      mock.onGet(`${format(OFFERING_ENDPOINTS.OFFERING, offeringId)}`).reply(HTTP_STATUS_CODES.OK, OFFERINGS_RESPONSE_1)
      await authenticateUser()
...
describe('Get starred offerings data', () => {
  ...
  test('when authenticated', async () => {
      const offerings = ["ac5b1727-629c-443b-8c1a-cc1bd541af6a", "2c7a83cc-e2f3-493a-ae65-33f9c998e8ed"];
      mock.onGet(`${format(OFFERING_ENDPOINTS.OFFERING, offerings[0])}`).reply(HTTP_STATUS_CODES.OK, OFFERINGS_RESPONSE_1)
      mock.onGet(`${format(OFFERING_ENDPOINTS.OFFERING, offerings[1])}`).reply(HTTP_STATUS_CODES.OK, OFFERINGS_RESPONSE_2)
      await authenticateUser()
